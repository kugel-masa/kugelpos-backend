...
- Python 3.12+（既存サービスと同一） + FastAPI、Pydantic v2、Motor（async MongoDB）、Dapr（pub/sub `pubsub-tranlog-report` / topic `topic-tranlog`、state store）。署名は phase 1 の `kugel_common.utils.hmac_signer`（標準ライブラリのみ）。リクエスト展開は標準ライブラリの `gzip`/`zlib` と `brotli` パッケージ（クライアント .NET 8 が `br`/`gzip` 標準対応） (156-stateless-cart)
