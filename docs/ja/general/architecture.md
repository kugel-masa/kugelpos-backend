...
**Redisキャッシュ:**
- マスターデータキャッシュ（Cart Service: 商品・決済・プロモーション・設定・税）
- 日次カウンター管理
- セッション管理

**Daprステートストア:**
- カート状態 — スナップショットを同梱しないリクエストのみ。issue #156 以降はクライアントが
  署名付きスナップショットとしてカートを保持し、同梱されたリクエストはストアを一切読まない。
  `carrySnapshot=true` で作られたカートはここに書かれない（#192）
- 冪等性キー保存
- 取引ステータス追跡
...
**スケーラビリティ:**
- 全サービスがステートレス設計。**カートも含む** — issue #156 以降はクライアントが署名付き
  スナップショットとしてカートを持ち回るため、どのバックエンドでもどのリクエストも処理できる
  （[クライアント持ち回りカート](../cart/client-carried-cart.md)）
- 水平スケーリング対応
- ロードバランサー対応
