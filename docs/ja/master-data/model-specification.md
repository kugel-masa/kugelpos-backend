...
2. 店舗レベル
3. グローバル（デフォルト値）
4. レコードが1件も無い場合、利用側サービス自身の設定値

手順 4 は API からは見えない（エンドポイントは、サービスが実際に使う値ではなく 404 を返す）。そのためテナント初期化時に、**POS 端末が読む必要のある設定**（`RECEIPT_NO_START_VALUE` / `RECEIPT_NO_END_VALUE`、issue #174）を既定値でシードし、これらの取得が必ず応答するようにしている。シードは insert-if-absent で、テナント初期化を再実行しても運用者が設定した値を上書きしない。なお一度シードすると値は明示的なレコードになるため、後からサービスの既定値を変更しても新規テナントにしか反映されない。

シードする値は、シップされた定数ではなく **master-data 自身の環境**（`.env` / 環境変数）から解決する。シードされたレコードはサービス自身の設定より優先されるため、環境変数でレンジを変更している配備では master-data にも同じ変数を設定するか、テナント設定を直接登録する必要がある。そうしないと、シードによってシップ既定値が設定値より優先されてしまう。
