...
"""Test health check endpoint for cart service."""
...
async def test_health_endpoint(http_client: AsyncClient):
    ...
    # Check checks structure - cart service checks MongoDB, Dapr components and background jobs
    assert "mongodb" in data["checks"]
    assert "dapr_sidecar" in data["checks"]
    assert "dapr_pubsub_tranlog" in data["checks"]
    assert "dapr_cartstore" in data["checks"]
