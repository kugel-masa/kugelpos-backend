...
async def register_invoice_registration_number(tenant_id, token):
    ...
    async with AsyncClient() as client:
        response = await client.post(
            f"{base_url}/tenants/{tenant_id}/settings",
            json=settings_master,
            headers=header
        )
...
async def setup_data_tranlog():
    ...
    target_db_name = f"{os.environ.get('DB_NAME_PREFIX')}_{tenant_id}"
    setup_db: AsyncIOMotorDatabase = db_client[target_db_name]
...
async def setup_data_invoice_number():
    """
