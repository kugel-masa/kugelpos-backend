...
async def clean_db(set_env_vars):
    ...
    )  #    databases = await db_client.list_database_names()  #    for db_name in databases:  #        if db_name.startswith(os.environ.get("DB_NAME_PREFIX")):  #            print(f"Dropping database: {db_name}")  #            await db_client.drop_database(db_name) #HACK: This is not working
