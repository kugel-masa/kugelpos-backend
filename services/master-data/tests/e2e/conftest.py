...
"""E2E test conftest for master-data service.

Drives the live master-data service (uvicorn at BASE_URL_MASTER_DATA)
and uses the live account service to fetch a real admin JWT. The full
...
async def _setup_master_data_db(set_env_vars):
    """Drop the master-data test DB once per session for e2e isolation."""
...
async def http_client(_reset_db_client_per_test):
    """AsyncClient pointed at the running master-data service."""
    base_url = os.environ.get("BASE_URL_MASTER_DATA", "http://localhost:8002")
...
async def _ensure_admin_user():
    """Register the admin user on the account service if not already
    present, so admin_token fixture can fetch a real JWT for TENANT_ID.
...
async def admin_token(_ensure_admin_user):
    """Fetch a real admin JWT from the running account service."""
    base_account = os.environ.get("BASE_URL_ACCOUNT", "http://localhost:8000")
    tenant_id = os.environ.get("TENANT_ID")
    async with AsyncClient(base_url=base_account, timeout=Timeout(timeout=None)) as client:
