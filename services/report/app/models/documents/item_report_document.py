...
class ItemReportDocument(AbstractDocument):
    """
    Document model representing an item-based sales report grouped by categories.
    
    This class defines the structure for storing aggregated sales data for individual items,
    organized by their product categories. It provides detailed insights into sales performance
    at the item level while maintaining category grouping for better organization.
    """
    
    class ItemReportItem(BaseDocumentModel):
        """
        Represents sales data for a specific item.
        
        Contains aggregated metrics for a single product item including
    ...
    class CategoryWithItems(BaseDocumentModel):
        """
        Represents a category containing multiple items.
        
        Groups items by category and includes category-level totals.
        """
        category_code: Optional[str] = None
        category_name: Optional[str] = None
        items: Optional[List["ItemReportDocument.ItemReportItem"]] = []
    ...
    report_type: Optional[str] = "item"
    
    # Category data with items
    categories: Optional[List[CategoryWithItems]] = []
