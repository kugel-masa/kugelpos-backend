"""
Integration tests for stock service with terminal JWT authentication.

Verifies that stock endpoints accept terminal JWT via Authorization header
...
@pytest.mark.asyncio
async def test_stock_with_jwt(http_client):
    """GET /stock with terminal JWT authentication."""
    tenant_id = os.environ.get("TENANT_ID")
    ...
    print(f"stock with JWT: {response.status_code} (auth OK)")


@pytest.mark.asyncio
async def test_stock_backward_compat_api_key(http_client):
    """GET /stock with API key (backward compatibility)."""
    tenant_id = os.environ.get("TENANT_ID")
    terminal_id = f"{tenant_id}-5678-9"
    ...
    print(f"stock with API key (backward compat): {response.status_code} (auth OK)")
