...
- `business_counter`: 端末の開設エポック（単調増加・非リセット、`terminal_service.py:429`）。**オフライン開設対応（Clarifications 2026-06-14、案 B）により端末所有・持ち回り**とし、open のたびに端末がローカルで前進。terminal service を耐久ホームとし open 時に `max(service値, 端末提示値)` reconcile（receipt_no と同一モデル、欠番許容・再利用禁止）。JWT クレーム / terminal_info 経由で持ち回る（既存 `tranlog.business_counter = terminal_info.business_counter`、`tran_service.py:165` を活用）。
...
**更新（Clarifications 2026-06-15）**: 乖離比較（`__comparable_cart_bytes`）は実装上 restore 経路にのみ存在していた。restore API 撤去（FR-010）に伴いこの比較コードも削除され、乖離検知は無効化される。これは本 R-008 の「キャッシュ撤去後は自然に無効化」と同方向の前倒しであり、A-1（1カート=1クライアント・乖離は異常系）かつ phase-1 クライアント不在の前提で許容する。キャッシュ自体（なし経路）は残置するが、乖離を観測する経路は持たない。
