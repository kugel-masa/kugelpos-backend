...
- [ ] T027 [US4] 端末交換・初期導入: open ごとの `business_counter` 取得で seq が新エポックから始まること、seq 復元処理が無いことを確認。**receipt_no（連続）の terminal service 連携**: open で receipt_no をシード（クレーム）、open 時に端末提示値と `max` で reconcile、端末は receipt_no を close→open 跨ぎで保持。交換＋オフライン未reconcile は tranlog 高水位照会 or 安全ジャンプ（欠番許容・再利用禁止）。terminal service の open 処理に max reconcile を実装（FR-012 / Clarifications 2026-06-14）
